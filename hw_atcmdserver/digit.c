...
NN_DIGIT a[2], b, c;
{
  ...
  bHigh = (NN_HALF_DIGIT)HIGH_HALF (b); // x >> 16
  bLow = (NN_HALF_DIGIT)LOW_HALF (b); // x & 0xFFFF
  cHigh = (NN_HALF_DIGIT)HIGH_HALF (c); // x >> 16
  cLow = (NN_HALF_DIGIT)LOW_HALF (c); // // x & 0xFFFF
  ...
  if ((t += u) < u)
    a[1] += TO_HIGH_HALF (1); // (4bytes)x << 16
  u = TO_HIGH_HALF (t);
