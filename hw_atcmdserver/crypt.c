...
/// NOTE: since length may not be known to the reader,
/// we follow the data with a non-hex character '.'
int sprint_hex_data(char* out_buf, struct DATA_BLOCK* block) {
    unsigned int i;
...
/// Stop when you reach a non-parsed character.
/// NOTE: buffer must be big enough; no checking is done.
static int sscan_hex_data(const char* p, struct DATA_BLOCK* block) {
    unsigned int m;
...
int print_key_hex(FILE* f, struct KEY* key, int size) {
    int len;
...
int scan_key_hex(FILE* f, struct KEY* key, int size) {
    unsigned int n;
    int len, i;
    int num_bits;
...
int sscan_key_hex(const char* buf, struct KEY* key, int size) {
...
int encrypt_private( R_RSA_PRIVATE_KEY* key, struct DATA_BLOCK* input, struct DATA_BLOCK* output) {
    int n, modulus_len;
    ...
    if (RSA_private_encrypt(n, input->data, output->data, rp, RSA_PKCS1_PADDING) < 0) {
        RSA_free(rp);
        return -1;
    }
    output->len = RSA_size(rp);
    RSA_free(rp);
    return 0;
}

int decrypt_public( R_RSA_PUBLIC_KEY* key,struct DATA_BLOCK* input,struct DATA_BLOCK* output) {
    RSA* rp = RSA_new();
    public_to_openssl(key, rp);
    if (RSA_public_decrypt(input->len, input->data, output->data, rp, RSA_PKCS1_PADDING) < 0) {
        RSA_free(rp);
        return -1;
    }
    output->len = RSA_size(rp);
...
/*
int sign_file(const char* path, R_RSA_PRIVATE_KEY* key,struct DATA_BLOCK* signature) {
...
/*
int sign_block(DATA_BLOCK& data_block, R_RSA_PRIVATE_KEY* key,struct DATA_BLOCK* signature) {
    char md5_buf[MD5_LEN];
    int retval;
    DATA_BLOCK in_block;
...
int generate_signature(
    ...
    struct DATA_BLOCK block, signature_data;
