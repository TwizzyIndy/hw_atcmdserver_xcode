...
int HWRSAPublicEncrypt(sec, secLen, output, outputLen, publicKey)
unsigned char* sec;
unsigned int secLen;
...

{
    ...
    if(secLen + 10 > modulusLen)
        return RE_LEN;
    
    pkcsBlock[0] = 0;
    pkcsBlock[1] = 1;
    
    for (i = 2; i < modulusLen - secLen - 1; i++) {
    ...
    R_memcpy ((POINTER)&pkcsBlock[i], (POINTER)sec, secLen);
